...
exports.api_songs = function(request, response) {
  ...
  // /api/songs?keyword=Jobim&bookcode=RBK1
  ...
  //by default order search results by song title and limit number of results
  let sql = `SELECT id, songs.title as title, page, songs.bookcode as bookcode, bookcodes.title as book_title
             FROM songs, bookcodes WHERE songs.bookcode = bookcodes.bookcode
                                         AND songs.bookcode LIKE ?
                                         AND (songs.title LIKE ? OR composer LIKE ?)
  ...
  if (urlObj.query['bookcode']) {
    ...
    //when searching a particular book order results by page number instead of title
    //and have no limit on the number of results -so we can see all the songs in a particular book
    sql = `SELECT id, songs.title as title, page, songs.bookcode as bookcode, bookcodes.title as book_title
               FROM songs, bookcodes WHERE songs.bookcode = bookcodes.bookcode
                                           AND songs.bookcode LIKE ?
                                           AND (songs.title LIKE ? OR composer LIKE ?)
               ORDER BY page ASC`
  }
  console.log("finding title or composer like keyword: " + title + " in book: " + bookcode)


  let result = {
    songs: []
  } //data object to send to client
  db.all(sql, bookcode, title, composer, function(err, rows) {
    for (let i = 0; i < rows.length; i++) {
      let song = {}
      song.id = rows[i].id
      song.title = rows[i].title
      song.page = rows[i].page
      song.bookcode = rows[i].bookcode
      song.book_title = rows[i].book_title
